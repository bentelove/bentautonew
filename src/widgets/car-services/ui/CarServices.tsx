      ...
      <div className="container px-4 mx-auto">
        ...
        <div className="flex gap-4">
            <div className="flex-1">
                ...
                {!modification?.generation?.image&&<Image className="rounded-lg" width={400} height={300} alt={`${modification?.generation?.model?.brand?.name} ${modification?.generation?.model?.name}`}  src={'https://cdn.lemanapro.ru/lmru/image/upload/c_pad/q_auto/f_auto/w_1000/h_1000/v1652787845/lmcode/DLZGMi6I6U-l-DH6pf0JnQ/90384094.jpg'}></Image>}
            </div>
            <div className="flex-3">
                <div className="bg-white px-4 py-4 border border-gray-200 rounded-lg">
                    ...
                    <div className="border border-gray-200">
                        ...
                        <CarService name={"Замена ремня вспомогательных агрегатов"} price={700}></CarService>
                    </div>
                    <h3 className='text-xl font-bold p-2'>Обслуживание трансмиссии  для {modification?.generation?.model?.brand?.name} {modification?.generation?.model?.name}</h3>
                    <div className="border border-gray-200">
                    ...
                    <h3 className='text-xl font-bold p-2'>Обслуживание тормозной системы для {modification?.generation?.model?.brand?.name} {modification?.generation?.model?.name}</h3>
                    <div className="border border-gray-200">
                    ...
                    <div className="flex mt-4">
                        ...
                        <div className="">
                            <button className="px-6 py-3 bg-gray-600 text-white rounded-lg">Рассчитать стоимость</button>
                        </div>
                    </div>
                </div>
            </div>
...
const CarService =({name,price}:CarServiceProps) =>{
    ...
    return <div className="flex items-center gap-2 bg-gray-100 even:bg-gray-50">
        <div className="flex-3 p-2">{name}</div>
        {price&&<div className="p-2 text-right">от {price} ₽</div>}
        <div className="p-2">
            {!check&&<div onClick={()=>{setCheck(true)}}><div className="h-8 w-8 bg-gray-200 hover:bg-gray-300 cursor-pointer rounded-full"></div></div>}
            {check&&<div onClick={()=>{setCheck(false)}}><div className="h-8 w-8 bg-red-600 hover:bg-red-700 cursor-pointer rounded-full"><div className="font-bold h-8 w-8 text-white text-center pt-1">+</div></div></div>}
        </div>
    </div>
}